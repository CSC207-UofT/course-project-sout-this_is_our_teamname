package DataCollection;

import TimeTableObjects.Parents.SearchingData;

import java.util.HashMap;

public abstract class DataGetter {
    private HashMap<String, SearchingData> data;

    public DataGetter(){
        this.data = new HashMap<>();
    }

    public void placeToData(String name, SearchingData theCourse) {
        this.data.put(name, theCourse);
    }

    abstract void CalibrateData(String cName);

    public HashMap<String, SearchingData> getData(String courseName){
        CalibrateData(courseName);
        return this.data;
    }
}
